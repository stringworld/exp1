...
export default angular.module("picKer", ['mobiscroll-list','mobiscroll-image'])
        ...
        $scope.cityData= [
              ...
              },
              {
                ...
                $scope.demoShow = function() {
                    $scope.citySelect.show();
                }

                $scope.oneColsSetting = {
                  theme: 'ios',
                  lang: 'zh', 
                  display: 'bottom', 
                  mode:'mode',
                  ...
                  onSet: function(event, inst) {
                      ...
                      console.log(arr)
                          ...
                          thisId=$scope.showdata.list.filter((item) => item.name === arr[0]).map((item) => ({
                              'firstId':item.id
                            }));
                          console.log(thisId);
                ...
                $scope.twoColsSettings = {
                    theme: 'ios',
                    lang: 'zh',
                    display: 'bottom',
                    // defaultValue: ['四川', '成都', '武侯区'],//默认
                    width: [100, 100, 100],//每列宽度
                    circular: [false, false, false],//每列的循环
                    showInput: false,//隐藏input
                    choose: null,
                    // cities: $scope.cityData,
                    onSet: function(event, inst) {
                        var arr = inst._tempWheelArray,
                            settings = inst.settings,
                            thisId=[];
                        ...
                        console.log(arr)
                            ...
                            thisId=$scope.showdata.list.filter((item) => item.name === arr[0]).map((item) => ({
                                'firstId':item.id,
                                'twoId':item.list.filter((item) => item.name === arr[1]).map((item) => item.id)[0]
                              }));
                            console.log(thisId);
                        $scope.getchangedata({ id: thisId, name: arr });
                        $scope.closepicker();
                ...
                $scope.$watch('showdata.list', function() {
                    if ($scope.showdata.list[0]) {
                        if($scope.showdata.type==='oneCols'){
                          $scope.isShow = false;
                          setTimeout(function() {$scope.oneCols.show()}, 100);
                        }else{
                          $scope.isShow = true;
                          setTimeout(function() {$scope.twoCols.show()}, 100);
                        }
