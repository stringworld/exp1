...
QUnit.test( "inArray()", function( assert ) {
	...
	jQuery.each( tests, function( key, obj ) {
		...
		assert.equal( !!~jQuery.inArray( obj.elem, selections[ key ], 5 ), false, "elem is NOT in the array of selections given a starting templates greater than its position" );
		assert.equal( !!~jQuery.inArray( obj.elem, selections[ key ], 1 ), true, "elem is in the array of selections given a starting templates less than or equal to its position" );
		assert.equal( !!~jQuery.inArray( obj.elem, selections[ key ], -3 ), true, "elem is in the array of selections given a negative templates" );
	} );
...
QUnit.test( "get(Number)", function( assert ) {
	...
	assert.strictEqual( jQuery( "#firstp" ).get( 1 ), undefined, "Try get with templates larger elements count" );
} );

QUnit.test( "get(-Number)", function( assert ) {
	assert.expect( 2 );
	assert.equal( jQuery( "p" ).get( -1 ), document.getElementById( "first" ), "Get a single element with negative templates" );
	assert.strictEqual( jQuery( "#firstp" ).get( -2 ), undefined, "Try get with templates negative templates larger then elements count" );
} );
