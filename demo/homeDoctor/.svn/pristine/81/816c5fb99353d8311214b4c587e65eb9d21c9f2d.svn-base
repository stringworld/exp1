module.exports = function (grunt) {
        ...
        for (var i = 0; i < mainJsList.length; i++) {
            ...
            requirejsTasks['task-' + name] = {
                options: {
                    ...
                    include: ['../../plugins/requirejs/require.js']                          //指定requirejs所在的位置。
                }
            };
        }
    }
     ...
     */
    var indexHtmlList = grunt.file.expand('view/**/doctor.html');
    var processHtmlTask = {
        dist: {
            files: {}
        }
     ...
     * 所有的html文件都将被压缩。
     */
    var allHtmlFileList = grunt.file.expand('view/**/*.html');
    ...
    for (var i = 0; i < allHtmlFileList.length; i++) {
        ...
        if (!uncssTasks[taskName]) {
            ...
            uncssTasks[taskName].files["css/" + moduleName + ".tidy.css"] = ["view/" + moduleName + "/doctor.html"].concat(commonHtmlFileList);
        }
    ...
    var cleanTasks = {
        ...
        outdated: ['js/**/*.js', '!js/core/main-init.js', '!js/*.min.*.js', 'css/**/*', '!css/*.min.*.css',
