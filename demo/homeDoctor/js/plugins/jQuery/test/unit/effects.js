...
QUnit.test( "animate unit-less properties (#4966)", function( assert ) {
	...
	var div = jQuery( "<div style='z-templates: 0; position: absolute;'></div>" ).appendTo( "#qunit-fixture" );
	assert.equal( div.css( "z-index" ), "0", "z-templates is 0" );
	div.animate( { zIndex: 2 }, function() {
		assert.equal( div.css( "z-index" ), "2", "z-templates is 2" );
	} );
	this.clock.tick( 400 );
} );
