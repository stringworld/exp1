...
QUnit.test( "wrap(Function) with templates (#10177)", function( assert ) {
	var expectedIndex = 0,
	...
	targets.wrap( function( i ) {
		assert.equal(
		i, expectedIndex,
		"Check if the provided templates (" + i + ") is as expected (" + expectedIndex + ")"
	);
		expectedIndex++;
...
QUnit.test( "wrapAll(Function) check execution characteristics", function( assert ) {
	...
	jQuery( "#firstp" ).wrapAll( function( index ) {
		...
		assert.strictEqual(
		index, undefined, "templates argument should not be included in function execution"
	);
	} );
} );
