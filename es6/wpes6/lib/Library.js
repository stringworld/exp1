...
/***/ function(module, exports, __webpack_require__) {
	...
	var p = new _test.person('zhang', 20);
	...
	function identify(identifier, reducer) {
	  return function identifiable(state, action) {
	    ...
	    var nextState = action.identifier === identifier ? reducer(state, action) : state;
	...
	var store = _redux2.default.createStore(rootReducer);
	...
	Array.from(document.getElementsByName('increment')).forEach(function (ele) {
	  ele.addEventListener('click', handleInc(ele.dataset['id']));
	});

	Array.from(document.getElementsByName('decrement')).forEach(function (ele) {
